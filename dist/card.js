...
function e(e2, r) {
  return (n, s, i) => {
    const o = (t) => {
      var _a, _b;
      return (_b = (_a = t.renderRoot) == null ? void 0 : _a.querySelector(e2)) != null ? _b : null;
    };
    return e$1(n, s, { get() {
      return o(this);
    } });
...
function mergeConfig(userConfig) {
  return __spreadProps(__spreadValues(__spreadValues({}, constants.DEFAULT_CONFIG), userConfig), {
    contacts: userConfig.contacts || constants.DEFAULT_CONFIG.contacts,
    stun_servers: userConfig.stun_servers || constants.DEFAULT_CONFIG.stun_servers,
    turn_servers: userConfig.turn_servers || constants.DEFAULT_CONFIG.turn_servers
  });
}
...
class SipManager extends EventTarget {
  ...
  async waitForSipLibrary(timeout = 15e3) {
    return new Promise((resolve, reject) => {
      ...
      const checkLibrary = () => {
        if (!Web) {
          Promise.resolve().then(() => require("./index.js")).then((SIP) => {
            Web = SIP.Web || SIP;
            continueCheck();
          }).catch(() => {
            setTimeout(checkLibrary, 200);
...
const sharedStyles = constants.i`
  :host {
    display: block;
...
exports.WebRTCSipCard = class WebRTCSipCard extends constants.i$1 {
  constructor() {
    super(...arguments);
  ...
  static getStubConfig() {
    return __spreadValues({}, constants.DEFAULT_CONFIG);
  }
  setConfig(config) {
    try {
  ...
  render() {
    if (!this.config) {
      return constants.x`<ha-card><div class="error">Configuration required</div></ha-card>`;
    }
    return constants.x` <ha-card> ${this.renderStatusBar()} ${this.renderError()} ${this.renderContent()} ${this.renderIncomingCallModal()} </ha-card> `;
  }
  renderStatusBar() {
    ...
    return constants.x`
      <div class="status-bar">
        <div class="status-indicator">
          ...
          ${this.isRetrying ? constants.x`<ha-circular-progress size="small" indeterminate></ha-circular-progress>` : ""}
        </div>
        <div class="card-title">${this.config.title || "WebRTC SIP Phone"}</div>
        ${!this.registered && !this.isRetrying ? constants.x`<ha-icon-button @click=${this.manualRetry} title="Retry connection">
  ...
  renderContent() {
    ...
    return constants.x` <div class="card-content">${this.renderContacts()} ${this.renderManualDial()} ${this.showKeypad ? this.renderKeypad() : ""}</div> `;
  }
  renderContacts() {
  ...
  renderKeypad() {
    return constants.x`
      <div class="keypad">
        ${constants.DTMF_KEYS.flat().map((key) => constants.x` <button class="keypad-button" @click=${() => this.handleKeypadPress(key)}>${key}</button> `)}
      </div>
    `;
  }
  renderActiveCall() {
  ...
  renderCallInfo() {
    return constants.x`
      <div class="call-info">
        ...
        ${this.callState.status === "answered" ? constants.x`<div class="call-duration">${this.callDuration}</div>` : ""}
      </div>
    `;
  }
  renderVideoStatus() {
  ...
  renderVideoArea() {
    return constants.x`
      <div class="video-container ${this.remoteVideoAvailable ? "has-video" : "no-video"}">
        ...
        ${!this.remoteVideoAvailable ? constants.x`
  ...
  renderInCallKeypad() {
    if (!this.showKeypad) return constants.x``;
    return constants.x`
      <div class="in-call-keypad">
        <div class="keypad">${constants.DTMF_KEYS.flat().map((key) => constants.x` <button class="keypad-button" @click=${() => this.sendDTMF(key)}>${key}</button> `)}</div>
      </div>
    `;
  }
...
__decorateClass([
  constants.n({ attribute: false })
], exports.WebRTCSipCard.prototype, "hass", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "config", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "connected", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "registered", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "error", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "callState", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "showKeypad", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "currentInput", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "callDuration", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "remoteVideoAvailable", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "connectionAttempts", 2);
__decorateClass([
  constants.r()
], exports.WebRTCSipCard.prototype, "isRetrying", 2);
__decorateClass([
  e("#remote-video")
], exports.WebRTCSipCard.prototype, "remoteVideoElement", 2);
exports.WebRTCSipCard = __decorateClass([
  constants.t("ha-webrtc-sip-card")
], exports.WebRTCSipCard);
...
window.customCards.push({
  ...
  version: constants.CARD_VERSION
});
console.info(
  `%c HA-WEBRTC-SIP-CARD %c ${constants.CARD_VERSION} `,
